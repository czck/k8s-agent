...
if (-not (Get-Command $plinkPath -ErrorAction SilentlyContinue)) {
    Write-Host "错误: 未找到 plink.exe，请确保 PuTTY 已安装并添加到系统 PATH 中" -ForegroundColor Red
    exit 1
}

if (-not (Get-Command $pscpPath -ErrorAction SilentlyContinue)) {
    Write-Host "错误: 未找到 pscp.exe，请确保 PuTTY 已安装并添加到系统 PATH 中" -ForegroundColor Red
    exit 1
}
