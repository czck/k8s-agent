...
try {
    ...
    # 上传 auto_healing_service.py 文件
    pscp -batch -pw $VM_PASSWORD "$PSScriptRoot\backend\services\auto_healing_service.py" "$VM_USER@$VM_HOST:$VM_DEPLOY_PATH/backend/services/"
    
    # 上传 verification_service.py 文件
    pscp -batch -pw $VM_PASSWORD "$PSScriptRoot\backend\services\verification_service.py" "$VM_USER@$VM_HOST:$VM_DEPLOY_PATH/backend/services/"
...
Write-Host "测试验证恢复接口: curl -X POST http://$VM_HOST:8000/api/verify-recovery -H \"Content-Type: application/json\" -d '{\"pod_name\":\"calico-node-k88st\",\"namespace\":\"calico-system\",\"executed_results\":[],\"plan\":{}}'" -ForegroundColor Yellow
Write-Host "测试自动修复接口: curl -X POST http://$VM_HOST:8000/api/auto-heal -H \"Content-Type: application/json\" -d '{\"pod_name\":\"calico-node-k88st\",\"namespace\":\"calico-system\",\"max_attempts\":3}'" -ForegroundColor Yellow
